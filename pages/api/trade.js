...
async function buyWithProtection(sym, qty, price, stop, target) {
  ...
  const order = {
    ...
    time_in_force: "day",
    order_class: "bracket",
    take_profit: { limit_price: target.toFixed(dec) },
    stop_loss: { stop_price: stop.toFixed(dec), limit_price: (stop * 0.99).toFixed(dec) },
  };
  try {
    const r = await fetch(`${ALPACA_BASE}/v2/orders`, { method: "POST", headers: H, body: JSON.stringify(order) });
    return r.json();
  } catch (e) {
  ...
  try {
    ...
    for (const pos of positions) {
      ...
      if (pnlPct >= CONFIG.TARGET_PROFIT * 100) {
        ...
        results.push({ symbol, action: "✅ هدف محقق", pnl: pnlPct.toFixed(2) + "%" });
        continue;
      }
      if (pnlPct <= -CONFIG.STOP_LOSS * 100) {
        await cancelAllOrders(symbol);
        await closePosition(symbol);
        results.push({ symbol, action: "🛑 وقف خسارة", pnl: pnlPct.toFixed(2) + "%" });
        continue;
      }
      if (pnlPct >= 0.3) {
        ...
        const order = {
          ...
          time_in_force: "day",
          order_class: "bracket",
          stop_loss: { stop_price: newStop.toFixed(2), limit_price: (newStop * 0.99).toFixed(2) },
        };
        await fetch(`${ALPACA_BASE}/v2/orders`, { method: "POST", headers: H, body: JSON.stringify(order) });
        results.push({ symbol, action: "🔒 تحديث الوقف", newStop: newStop.toFixed(2) });
      }
    }
  } catch (error) {
...
export default async function handler(req, res) {
  ...
  const log = { entered: [], skipped: [], managed: [], errors: [] };

  try {
    // ❌ تم إزالة التحقق من CRON_SECRET بالكامل
    ...
    if (day === 0 || day === 6) {
      return res.status(200).json({ success: true, message: "⏸️ عطلة نهاية الأسبوع" });
    }

    if (mins < 570 || mins > 960) {
      return res.status(200).json({ success: true, message: "⏳ خارج ساعات التداول" });
    }
    ...
    if (balance <= 0) {
      return res.status(200).json({ success: true, message: "⚠️ رصيد غير كافٍ" });
    }

    // ─── جلب الإشارات من Supabase ──────────────────────
    const todayET = new Date().toISOString().split("T")[0];
    const url = `${SUPABASE_URL}/rest/v1/signals?select=*&signal_date=eq.${todayET}&order=score.desc&limit=100`;
    const sr = await fetch(url, { headers: SB_H });
    if (!sr.ok) throw new Error(`Supabase error: ${sr.status}`);
    const signals = await sr.json();

    if (!Array.isArray(signals) || signals.length === 0) {
      return res.status(200).json({ success: true, message: "📭 لا توجد إشارات اليوم" });
    }
