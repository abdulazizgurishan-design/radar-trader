...
async function fetchAccount() {
  const r = await fetch(`${ALPACA_BASE}/v2/account`, { headers });
  return r.json();
}

async function fetchPositions() {
  const r = await fetch(`${ALPACA_BASE}/v2/positions`, { headers });
  return r.json();
}

async function fetchOrders() {
  const r = await fetch(`${ALPACA_BASE}/v2/orders?status=closed&limit=20`, { headers });
  return r.json();
}
...
export default function Dashboard() {
  ...
  const runTrade = async () => {
    ...
    try {
      ...
      setTradeMsg(d.tradesPlaced > 0
        ? `✅ نُفذت ${d.tradesPlaced} صفقة`
        : `⚠️ ${d.message || "لا توجد فرص"}`
      );
      await refresh();
    } catch {
      setTradeMsg("❌ خطأ في الاتصال");
    } finally {
      setTrading(false);
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: "#080c18", color: "#fff", fontFamily: "system-ui", direction: "rtl", padding: "20px 16px 40px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 4 }}>
          {lastUpdate ? `آخر تحديث: ${lastUpdate.toLocaleTimeString("ar")}` : "جاري التحميل..."}
        </div>
      </div>
          ...
          return (
            <div key={p.symbol} style={{ background: "linear-gradient(135deg,rgba(15,20,35,0.95),rgba(20,28,48,0.95))", border: `1px solid ${isUp ? "rgba(0,212,170,0.2)" : "rgba(255,71,87,0.2)"}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10, borderRight: `3px solid ${isUp ? "#00d4aa" : "#ff4757"}` }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[
                  { label: "الكمية", value: p.qty },
                  { label: "سعر الدخول", value: `$${fmt(p.avg_entry_price)}` },
                  { label: "السعر الحالي", value: `$${fmt(p.current_price)}` },
                  { label: "القيمة", value: `$${fmt(p.market_value)}` },
                ].map(m => (
      ...
      <p style={{ textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.1)", marginTop: 24 }}>
        Paper Trading · لا فلوس حقيقية · يتحدث كل 30 ثانية
      </p>
    </div>
  );
}
